...
var _squel = __webpack_require__(3);

var Squel = _interopRequireWildcard(_squel);

var _json2Sql = __webpack_require__(1);

var _json2Sql2 = _interopRequireDefault(_json2Sql);
...
var CreateIndexBlock = function (_Squel$cls$Block) {
  _inherits(CreateIndexBlock, _Squel$cls$Block);

  function CreateIndexBlock() {
    _classCallCheck(this, CreateIndexBlock);

    return _possibleConstructorReturn(this, (CreateIndexBlock.__proto__ || Object.getPrototypeOf(CreateIndexBlock)).apply(this, arguments));
  }

  _createClass(CreateIndexBlock, [{
    key: 'addIndexDirectives',
    value: function addIndexDirectives(directives) {
      var column = '';
    ...
    key: '_buildIndexList',
    value: function _buildIndexList(list) {
      if (list) {
        return '(' + list.join() + ')';
      }
      return '';
    ...
    key: '_buildJoinFor',
    value: function _buildJoinFor(value) {
      var joinFor = '';
  ...
  return CreateIndexBlock;
}(Squel.cls.Block);

var SqlGenerators = function () {
  function SqlGenerators() {
    _classCallCheck(this, SqlGenerators);

    this.select = null;

    Squel.myselect = function (options) {
      return Squel.select(options, [new Squel.cls.StringBlock(options, 'SELECT'), new Squel.cls.FunctionBlock(options), new Squel.cls.DistinctBlock(options), new Squel.cls.GetFieldBlock(options), new Squel.cls.FromTableBlock(options), new CreateIndexBlock(options), new Squel.cls.JoinBlock(options), new Squel.cls.WhereBlock(options), new Squel.cls.GroupByBlock(options), new Squel.cls.HavingBlock(options), new Squel.cls.OrderByBlock(options), new Squel.cls.LimitBlock(options), new Squel.cls.OffsetBlock(options), new Squel.cls.UnionBlock(options)]);
    };
  ...
  _createClass(SqlGenerators, [{
    key: 'handleParameterizedSources',
    value: function handleParameterizedSources(datasources) {
      ...
      if (Array.isArray(datasources)) {
        for (var i = 0; i < datasources.length; i++) {
          ...
          if (source.table && source.table.indexOf('<<') >= 0) {
            var variable = this._getVariable(source.table);
          ...
          if (source.dataSet && source.dataSet.indexOf('<<') >= 0) {
            var _variable = this._getVariable(source.dataSet);

            if (params[_variable]) {
              source.dataSet = this._replaceVariable(source.dataSet, _variable, params[_variable]);
            }
          }
        }
      }
    ...
    key: '_getVariable',
    value: function _getVariable(expression) {
    ...
    key: '_replaceVariable',
    value: function _replaceVariable(expression, variableName, value) {
      return expression.replace('<<' + variableName + '>>', value);
    }
  }, {
    key: 'generateColumns',
    value: function generateColumns(columns) {
      var params = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};

      var filteredCoulmn = columns;
      ...
      try {
        for (var _iterator2 = filteredCoulmn[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
          var column = _step2.value;

          if (column.type === 'simple_column') {
            this.select.field(column.column, column.alias);
          } else if (column.type === 'derived_column' && column.expressionType === 'simple_expression' && column.expressionOptions && column.expressionOptions.expression) {
            var injectedExpression = this._stringInject(column.expressionOptions.expression, params);
    ...
    key: 'generateWhere',
    value: function generateWhere(filters, params) {
      var _iteratorNormalCompletion3 = true;
      var _didIteratorError3 = false;
      var _iteratorError3 = undefined;

      try {
        for (var _iterator3 = filters.conditions[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
          var condition = _step3.value;

          if (condition.conditionExpression) {
            var injectedExpression = this._stringInject(condition.conditionExpression, params);
            var param = params[condition.parameterName];
    ...
    key: 'generateGroupBy',
    value: function generateGroupBy(groupBy, params) {
      var groupParams = params[groupBy.groupParam] || [];
      var excludeParam = params[groupBy.excludeParam];

      if (groupParams.length > 0 && groupBy.override) {
        this._addGroupColumns(this.select, groupParams, excludeParam);
        return this;
      }
      this._addGroupColumns(this.select, [].concat(_toConsumableArray(new Set(groupBy.columns.concat(groupParams)))), excludeParam);
      return this;
    }
  }, {
    key: 'generateOrderBy',
    value: function generateOrderBy(orderBy, params) {
      var orderParams = params[orderBy.orderByParam] || [];
    ...
    key: 'generatePaging',
    value: function generatePaging(paging, params) {
      if (paging) {
        var limit = params[paging.limitParam];
        var offset = params[paging.offSetParam];
    ...
    key: 'generateCase',
    value: function generateCase(caseObject, params) {
      var squelCase = Squel.case();
      ...
      if (caseObject && caseObject.expressionOptions && caseObject.expressionOptions.caseOptions) {
        ...
        try {
          for (var _iterator4 = caseObject.expressionOptions.caseOptions[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
    ...
    key: 'generateDataSources',
    value: function generateDataSources(dataSources, dataSets, params) {
      var firstRun = true;
      var selectSubquery = null;
      ...
      try {
        for (var _iterator5 = dataSources[Symbol.iterator](), _step5; !(_iteratorNormalCompletion5 = (_step5 = _iterator5.next()).done); _iteratorNormalCompletion5 = true) {
          var dataSource = _step5.value;

          params = SqlGenerators.mapParams(dataSource, params);
          if (dataSource.dataSet && dataSets) {
            var json2sql = new _json2Sql2.default(dataSets[dataSource.dataSet], dataSets, params);
          ...
          if (!firstRun && dataSource.join) {
            var joinType = dataSource.join.type.toUpperCase();
            var joinExpression = this._stringInject(dataSource.join.joinCondition, params);
    ...
    key: '_addGroupColumns',
    value: function _addGroupColumns(select, columns) {
    ...
    key: '_addOrderColumns',
    value: function _addOrderColumns(select, columns) {
    ...
    key: '_stringInject',
    value: function _stringInject(str, data) {
    ...
    key: 'mapParams',
    value: function mapParams(dataSource, params) {
      if (dataSource.forwardedParams) {
        ...
        try {
          for (var _iterator8 = dataSource.forwardedParams[Symbol.iterator](), _step8; !(_iteratorNormalCompletion8 = (_step8 = _iterator8.next()).done); _iteratorNormalCompletion8 = true) {
            var param = _step8.value;


            var paramsToMap = param.mapping.split(':');
    ...
    key: 'mapParamsMultipleDataSources',
    value: function mapParamsMultipleDataSources(dataSources, params) {
      var _iteratorNormalCompletion9 = true;
      var _didIteratorError9 = false;
      var _iteratorError9 = undefined;

      try {
        for (var _iterator9 = dataSources[Symbol.iterator](), _step9; !(_iteratorNormalCompletion9 = (_step9 = _iterator9.next()).done); _iteratorNormalCompletion9 = true) {
...
var _sqlGenerators = __webpack_require__(0);

var _sqlGenerators2 = _interopRequireDefault(_sqlGenerators);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }

var Json2Sql = function () {
  function Json2Sql(schema, dataSets, params) {
    _classCallCheck(this, Json2Sql);
  ...
  _createClass(Json2Sql, [{
    key: 'generateSQL',
    value: function generateSQL() {
      var generate = new _sqlGenerators2.default();
...
exports.Json2Sql = exports.SqlGenerators = undefined;
...
exports.SqlGenerators = _sqlGenerators2.default;
exports.Json2Sql = _json2Sql2.default;
...
//# sourceMappingURL=ampath-json2sql.js.map